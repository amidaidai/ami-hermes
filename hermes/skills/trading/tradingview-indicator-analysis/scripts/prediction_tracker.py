...
用法：
  python prediction_tracker.py stats       # 胜率看板
  python prediction_tracker.py verify BTCUSDT 65192 4   # 回验4h前预测
  python prediction_tracker.py log 65000    # 记录测试预测
"""
...
def verify_predictions(symbol: str, current_price: float, lookback_hours: float = 4.0) -> list:
    ...
    for line in lines:
        ...
        if pred_time > cutoff or pred["symbol"] != symbol:
            updated_lines.append(json.dumps(pred, ensure_ascii=False, separators=(",", ":")))
            continue
        ...
        if direction == "偏多":
            was_correct = price_change > 0.001
        elif direction == "偏空":
            was_correct = price_change < -0.001
        else:
...
def aggregate_stats() -> dict:
    ...
    result = {
        ...
        "overall_accuracy": round(sum(1 for p in predictions if p["was_correct"]) / len(predictions), 3) if predictions else 0,
        "models": {},
    }
    for name, s in sorted(model_stats.items(), key=lambda x: -x[1]["total"]):
        result["models"][name] = {
            "predictions": s["total"],
            "correct": s["correct"],
            "accuracy": round(s["correct"] / s["total"], 3) if s["total"] > 0 else 0,
...
if __name__ == "__main__":
    ...
    if action == "stats":
        ...
        pred = {
            ...
            "direction": "偏多",
            "long_confidence": 0.65, "short_confidence": 0.45,
            "global_confidence": 0.65, "action": "可交易 · 轻仓",
            "models": {"Taker背离": {"dir": "long", "conf": 0.8}},
            "verified": False,
        }
        DATA.mkdir(parents=True, exist_ok=True)
        with open(PRED_FILE, "a", encoding="utf-8") as f:
