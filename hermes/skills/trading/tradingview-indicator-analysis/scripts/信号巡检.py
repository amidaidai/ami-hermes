...
def maybe_run_99_maintenance(state: dict[str, Any]) -> list[str]:
    ...
    if now_ts - last_health > HEALTH_REFRESH_SECONDS:
        try:
            subprocess.run([sys.executable, str(SCRIPT_DIR / "模型统计.py")], capture_output=True, text=True, timeout=30)
            subprocess.run([sys.executable, str(SCRIPT_DIR / "安全审计.py")], capture_output=True, text=True, timeout=45)
            subprocess.run([sys.executable, str(SCRIPT_DIR / "系统体检.py")], capture_output=True, text=True, timeout=60)
            state["last系统体检"] = now_ts
            outputs.append("模型统计、安全审计和系统体检已刷新")
        except Exception as e:
...
def main() -> None:
    ...
    if new_events:
        ...
        print(render_event(new_events[0], levels))
        return
    if update_output:
    ...
    if maintenance and not findings:
