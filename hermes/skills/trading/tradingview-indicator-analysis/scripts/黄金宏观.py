...
def bias_from_changes(dxy_chg, ten_chg, gold_chg, tip_chg=None, silver_chg=None, miners_chg=None, move_chg=None) -> tuple[str, list[str]]:
    ...
    if isinstance(gold_chg, (int, float)):
        if gold_chg > 0.15:
            score += 1; notes.append("GC期货同步偏强")
        elif gold_chg < -0.15:
            score -= 1; notes.append("GC期货同步偏弱")
        else:
            notes.append("GC期货同步温和")
...
def main() -> None:
    ...
    row = {
        ...
        "changes_pct": changes,
        "notes": notes,
        "rule": "DXY和美债上行压制黄金；DXY和收益率回落支撑黄金；TIP/TLT作实际利率与久期代理；GC/MGC、白银、GLD/GDX作贵金属同步确认；MOVE提示利率波动风险",
    }
