...
输入：模型类型 + 方向 + 关键价格（JSON context）
输出：通用入口 + 模型特定 checklist 逐项勾选结果。

用法：
  python scripts/model_checklist.py --model "POC拒绝" --context '{...json...}'
"""
...
COMMON_CHECKS = [
    ...
    {"id": "rr_min", "label": "R:R≥1:2可构造", "fn": lambda ctx: ctx.get("rr", 0) >= 2.0},
...
def run_checklist(model_name, context):
    results = {"model": model_name, "common": [], "model_specific": {}, "all_pass": True}
    for check in COMMON_CHECKS:
        passed = check["fn"](context)
        results["common"].append({"id": check["id"], "label": check["label"], "pass": passed})
        if not passed:
...
def format_checklist(results):
    ...
    for phase, checks in results["model_specific"].items():
        ...
        for c in checks:
            lines.append(f"  {'☑' if c['pass'] else '☐'} {c['label']}")
    lines.append(f"\n总体：{'全部通过 → 可执行' if results['all_pass'] else '未全部通过 → B等待或X禁做'}")
    return "\n".join(lines)
