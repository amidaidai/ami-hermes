...
| 档位 | 用户说的 | 自动触发 | 跑什么 | 调用数 | 耗时 |
|:--:|---------|---------|---------|:--:|:--:|
| **L1 轻量** | `看下BTC` / `看下XAU` / `看一眼`（默认） | ⚡ 轻量 | 行动格 table + quote 现价 + 一张截图（加密另加衍生品 curl） | **~5** | ~30s |
| **L2 标准** | `现在呢` / `更新` / `扫一下` — 追踪类 | 🔀 标准 | L1 + 上下邻居周期只读行动格结论行 + 关键位表 | ~8 | ~1.5min |
| **L3 完整** | `分析` / `分析BTC` / `分析XXX` | 🔍 完整 | router full mode（全管线） | 全量 | ~3min |
