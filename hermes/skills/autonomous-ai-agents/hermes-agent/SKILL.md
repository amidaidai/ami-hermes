...
> **Script path resolution, sleep-hour scheduling, and common pitfalls:**
> `skill_view(name="hermes-agent", file_path="references/cron-management-pitfalls.md")`
...
**Postinstall scripts skipped by Hermes desktop runtime npm.** When Hermes Web UI's desktop runtime installs npm packages globally (e.g. `@anthropic-ai/claude-code`), the npm invocation may use `--omit=optional` or `--ignore-scripts`, leaving native binary stubs (500-byte placeholder scripts) instead of the real executables. The error manifests as "this version is not compatible with this Windows version" when trying to run the stub. Fix: navigate to the package directory and manually run the postinstall:

```
cd <runtime-node>/node_modules/@anthropic-ai/claude-code
node install.cjs
```

Where `<runtime-node>` is the Hermes desktop runtime's bundled Node (e.g. `C:\Users\<user>\.hermes-web-ui\desktop-runtime\hermes\<version>\win-x64\node`). The postinstall script resolves the platform-specific optional dependency, hardlinks/copies the real native binary (~235MB) over the placeholder stub. This applies to any npm package that uses `postinstall` to place a native binary from an `optionalDependencies` package.
