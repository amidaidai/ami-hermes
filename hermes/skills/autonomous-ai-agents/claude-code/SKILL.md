...
- **Windows postinstall fix:** Hermes bundler's npm installs the package, but the global `claude` shim may be a shell script (308-byte POSIX script) rather than a native Windows binary. **Verify** with `file $(which claude)` — if it says "POSIX shell script", apply the fix:
  ```bash
  cd $(npm root -g)/@anthropic-ai/claude-code && node install.cjs
  ```
  This runs the postinstall script which copies the 230MB native exe into `bin/claude.exe`. **Note:** the binary may ALREADY be present at `$(npm root -g)/@anthropic-ai/claude-code/bin/claude.exe` even if the global `claude` shim is still a shell script — the shim correctly references the exe path in most cases. Run `claude --version` — if it works, the shell shim is functional.
  
  If the win32 optional dependency was NOT downloaded (npm used `--omit=optional`), you'll see a 404 or the file won't exist. Fix: reinstall with `npm install -g @anthropic-ai/claude-code --include=optional` or manually install `npm install -g @anthropic-ai/claude-code-win32-x64`.
  
  **Post-install verification pipeline** (in this order):
  ```bash
  which claude                      # 1. Confirm it's on PATH
  file $(which claude)              # 2. Check: "shell script" vs "PE32 executable"
  claude --version                  # 3. Basic function check (exits fast)
  claude -p "hello" --max-turns 1  # 4. Real functional test (omit --max-budget-usd or set ≥ $0.10)
  claude auth status --text         # 5. Check auth + API endpoint + proxy
  ```
  ⚠️ **Do NOT use `claude doctor` as primary health check** — it performs network probes and can hang for >30 seconds without producing useful diagnostic info. Use the pipeline above instead.
...
3. **`--max-budget-usd` minimum is ~$0.10** — system prompt cache creation alone costs ~$0.21 for opus. Setting too low (e.g., $0.05) triggers `error_max_budget_usd`. Use `--max-budget-usd 0.25` for comfort if you must cap spend.
...
13. **`claude doctor` can hang indefinitely** — especially through a proxy or custom API endpoint. It probes auto-updater and connectivity. Use `claude --version` + `claude -p "hello" --max-turns 1` as health checks instead.
14. **Custom API base URLs (`ANTHROPIC_BASE_URL`)** — users routing through custom proxies (ccapi.us, right.codes, timicc.com, etc.) must ensure the endpoint supports Claude Code's specific API version. The `ANTHROPIC_AUTH_TOKEN` env var takes precedence over OAuth for these setups.
