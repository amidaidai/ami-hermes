...
- **SkillSpector (NVIDIA)** — Automated static analysis of AI agent skill directories. Detects 64 vulnerability patterns across 16 categories: prompt injection, data exfiltration, supply chain, MCP tool poisoning, etc. Outputs 0-100 risk score with per-issue reports. See [references/skillspector.md](references/skillspector.md) for installation, usage, and interpretation guide.
