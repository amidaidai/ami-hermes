...
def render_card_locked(symbol: str, merged: dict, results: list[dict], meta: dict,
    ...
    if status == "B等待" or direction == "wait":
        ...
        ops.append(f"④ 仓位：`{_qty(price, meta, bias_cn)} BTC` · 风险 `{meta.get('risk_usd',2)}U` · 名义 `{_notional(price, meta, bias_cn)}U`")
        ...
        ops.append(f"④ 仓位：`{_qty_b(price, meta, bias_cn)} BTC` · 风险 `{meta.get('risk_usd',2)}U` · 名义 `{_notional_b(price, meta, bias_cn)}U`")
        ...
        ops.append(f"④ 仓位：`{_qty(price, meta, bias_cn)} BTC` · 风险 `{meta.get('risk_usd',2)}U`")
