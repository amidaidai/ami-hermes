...
def compute_dmi(highs: list[float], lows: list[float], closes: list[float],
    ...
    di_minus_list = []
    for i in range(n):
        denom = tr_rma[i] if tr_rma[i] > 0 else 1.0
        di_plus_list.append(100.0 * plus_rma[i] / denom)
        di_minus_list.append(100.0 * minus_rma[i] / denom)

    dx_list = []
    for i in range(n):
        s = di_plus_list[i] + di_minus_list[i]
        dx_list.append(100.0 * abs(di_plus_list[i] - di_minus_list[i]) / (s if s > 0 else 1.0))

    adx_list = rma(dx_list, adx_smooth)

    latest_adx = adx_list[-1] if adx_list[-1] > 0 else 50.0
    latest_dip = di_plus_list[-1]
    latest_dim = di_minus_list[-1]
    ...
    bull_against = bear or (latest_dim > latest_dip and gap > 3.0)
    bear_against = bull or (latest_dip > latest_dim and gap > 3.0)
    ...
    return {
        ...
        "di_plus": round(latest_dip, 1),
        "di_minus": round(latest_dim, 1),
        "gap": round(gap, 1),
        "balance": balance,
        "bull": bull,
        "bear": bear,
        ...
        "bull_against": bull_against,
        "bear_against": bear_against,
...
def f_near_level(price: float, level: Optional[float], atr: float, mult: float) -> bool:
    """Check if price is within mult * ATR of a level."""
    if level is None or atr <= 0:
        return False
    return abs(price - level) <= atr * mult


def compute_scores(data: dict, dmi: dict, atr: float, ohlcv_bars: list[dict]) -> dict:
    """
    Compute trend score and reversal score — exact replica of Pine indicator logic.

    Returns full decision state dict.
    """
    ...
    val = data.get("val", 0)
    poc = data.get("poc", 0)
    b1h = data.get("band1_high", 0)
    b1l = data.get("band1_low", 0)
    ...
    w_vwap = data.get("w_vwap", 0)
    m_vwap = data.get("m_vwap", 0)
    ...
    near_poc = poc > 0 and atr > 0 and abs(price - poc) <= atr * 0.20
    near_s = vwap > 0 and atr > 0 and abs(price - vwap) <= atr * 0.20
    ...
    cvd_falling = cvd_slope < 0
    cvd_bull_confirm = cvd_rising and price >= price  # simplified — needs close comparison
    cvd_bear_confirm = cvd_falling and price <= price  # simplified

    # More accurate CVD states from bars
    slope_lookback = 5
    cvd_bull_confirm = False
    cvd_bear_confirm = False
    cvd_bull_div = False
    cvd_bear_div = False
    cvd_long_ok = True
    cvd_short_ok = True
    ...
    if ohlcv_bars and len(ohlcv_bars) >= 20:
        ...
        # Simple CVD confirmation: slope direction matches price direction
        if len(closes_bars) >= slope_lookback:
            price_up_5 = closes_bars[-1] > closes_bars[-slope_lookback]
            cvd_bull_confirm = cvd_rising and price_up_5
            cvd_bear_confirm = cvd_falling and not price_up_5

        # CVD divergence: price new extreme but CVD not confirming
        if len(highs_bars) >= 20 and cvd != 0:
            prev_high = max(highs_bars[:-1])
            if highs_bars[-1] >= prev_high and cvd < 0:
                cvd_bear_div = True
            prev_low = min(lows_bars[:-1])
            if lows_bars[-1] <= prev_low and cvd > 0:
                cvd_bull_div = True
    ...
    cvd_state = ("顶背离" if cvd_bear_div else "底背离" if cvd_bull_div else
    ...
    if ohlcv_bars:
        ...
        if volumes:
            ...
            rel_vol = vol_recent / vol_ma if vol_ma > 0 else 1.0
    ...
    # ── HTF (high timeframe) filter ──
    # Simplified: use weekly VWAP as proxy. If price above both VWAP and W_VWAP, HTF bull.
    htf_bull = vwap > 0 and w_vwap > 0 and price >= vwap and price >= w_vwap and ema9 >= ema55 if (ema9 and ema55) else False
    htf_bear = vwap > 0 and w_vwap > 0 and price < vwap and price < w_vwap and ema9 < ema55 if (ema9 and ema55) else False
    htf_allow_long = htf_bull or not htf_bear
    htf_allow_short = htf_bear or not htf_bull

    # ── ICT sweep detection (simplified from labels) ──
    swept_low_reclaimed = False
    swept_high_rejected = False
    ...
    # ── VAH reject / VAL reclaim ──
    vah_rejected = vah > 0 and price < vah
    val_reclaimed = val > 0 and price > val

    # ── Key level proximity ──
    key_levels = [l for l in [vah, val, poc, vwap, b1h, b1l] if l and l > 0]
    near_key_vp = any(f_near_level(price, l, atr, 0.45) for l in [vah, val, poc] if l)
    near_key_vwap = any(f_near_level(price, l, atr, 0.45) for l in [vwap, b1h, b1l] if l)
    near_key_level = near_key_vp or near_key_vwap

    near_a_key_vp = any(f_near_level(price, l, atr, 0.60) for l in [vah, val, poc] if l)
    near_a_key_vwap = any(f_near_level(price, l, atr, 0.60) for l in [vwap, b1h, b1l] if l)
    near_a_key = near_a_key_vp or near_a_key_vwap
    ...
    # absorption buy (simplified)
    if cvd < -1000 and price > (min([b.get("low", price) for b in ohlcv_bars[-12:]]) if ohlcv_bars else price):
        reversal_long += 1
    ...
    # distribution sell
    if cvd > 1000 and price < (max([b.get("high", price) for b in ohlcv_bars[-12:]]) if ohlcv_bars else price):
        reversal_short += 1
    ...
    # ═══════════════ GRADE ═══════════════
    # Acceptance check
    accept_above_s = vwap > 0 and price > vwap
    accept_below_s = vwap > 0 and price < vwap
    acceptance_bull_ok = accept_above_s and (price_above_vah or price_in_va)
    acceptance_bear_ok = accept_below_s and (price_below_val or price_in_va)
    ...
    # Direction
    if grade_raw in ("A", "B") and setup_long_a or setup_long_b:
        bias = "偏多"
    elif grade_raw in ("A", "B") and setup_short_a or setup_short_b:
        bias = "偏空"
    elif setup_long_c:
        bias = "反多"
    elif setup_short_c:
    ...
    return {
        ...
        "ema_bear": ema_bear,
        "cvd_state": cvd_state,
        "cvd_bull_confirm": cvd_bull_confirm,
        "cvd_bear_confirm": cvd_bear_confirm,
        "cvd_bull_div": cvd_bull_div,
        "cvd_bear_div": cvd_bear_div,
        "price_above_s": price_above_s,
        "price_above_vah": price_above_vah,
        "price_below_val": price_below_val,
        "price_in_va": price_in_va,
        "near_key_level": near_key_level,
        "near_a_key": near_a_key,
        ...
        "x_htf_conflict": x_htf_conflict,
        "setup_x": setup_x,
        "setup_long_a": setup_long_a,
