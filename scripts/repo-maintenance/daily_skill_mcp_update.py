...
def _hermes_cmd(*args: str) -> list[str]:
    """构建可靠的 hermes CLI 调用（避免 uv trampoline 路径问题）"""
...
def check_skills_update() -> dict:
    """检查并更新 hub skills"""
    code, out = run(_hermes_cmd("skills", "check"), timeout=60)
...
def run_skills_update() -> dict:
    """执行技能更新"""
    code, out = run(_hermes_cmd("skills", "update"), timeout=180)
    return {
...
def run_curator() -> dict:
    """运行 Curator 维护"""
    code, out = run(_hermes_cmd("curator", "run"), timeout=120)
    # 检查是否做了事
