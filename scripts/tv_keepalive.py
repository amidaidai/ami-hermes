...
# 注意：%LOCALAPPDATA%\TradingView 才是 MCP/launch 的实际启动位，必须靠
# tv_sync_appdir.py 与 Store(WindowsApps) 版本保持同步，否则会一直拉起旧副本。
...
def try_launch() -> bool:
    ...
    try:
        ...
        if res.get("status") == "synced":
            log(f"⬆ TV 启动目录已同步到 Store 版 {res.get('store_version')}")
    except Exception:
